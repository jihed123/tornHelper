...
function retrieveData() {
	return browser.storage.local
		.get(["numberOfAPIRequests", "lastResetTime", "apiKey", "margin"])
		.then((result) => {
...
async function fetchPriceData(itemid) {
	...
	if (priceData[itemid] === undefined) {
		// TODO: add all items to priceData so i can filter by new items and make this section relevent
		console.log("Item not found in priceData : ", itemid);
		return "notFound";
	}
		...
		await response.json().then((data) => {
			...
			if (data.itemmarket != undefined) {
				if (data.itemmarket[0].cost < priceData[itemid]) {
					if (priceData[itemid] - data.itemmarket[0].cost > margin) {
						value = "profitable";
					}
			...
			if (data.bazaar != undefined) {
				if (data.bazaar[0].cost < priceData[itemid]) {
					if (priceData[itemid] - data.bazaar[0].cost > margin) {
						value = "profitable";
					}
